...
sudo apt -y install curl git nano zsh gwet htop etherape snapd
...
# install kali repos kali default repo echo "deb 
#http://http.kali.org/kali kali-rolling main non-free contrib" | 
#sudo tee /etc/apt/sources.list kali-last-snapshot (Point release 
#so more “stable” & the “safest”) ​ echo "deb 
#http://http.kali.org/kali kali-last-snapshot main non-free 
#contrib" | sudo tee /etc/apt/sources.list kali-experimental 
#(Packages which are under testing - often used with the rolling 
#repository) ​ echo "deb http://http.kali.org/kali kali-experimental 
#main non-free contrib" | sudo tee -a /etc/apt/sources.list
